...
char* _str[] =
{
    ...
    "ERR:Unexpected Token:\"%.100s\"  at line:%d col:%d",
...
mHashtable * new_mHashtable(size_t start_size, mhash_f hsh_f, mcmp_f cmp_f)
{
    ...
    if(!res->tbl) goto ERR2;
    res->lstn=0;
    res->lstsz=start_size;
    res->lst=calloc(res->lstsz,sizeof(mHList));
    if(!res->lst) goto ERR3;
    if (! (res->free_slots=new_mStack(10))) goto ERR4;
    res->hsh_f=hsh_f;
    res->cmp_f=cmp_f;
    res->fk_f=NULL;
    res->fv_f=NULL;
...
void destroy_mHashtable(mHashtable * tbl,mfree_mvalue_f fkey,mfree_mvalue_f fval)
{
    ...
    if( (tbl->flags &(MSHS_FREE_STR_KEY_ON_DESTROY|MSHS_FREE_STR_VALUE_ON_DESTROY))
         || fkey|| fval)
        while(tbl->lstn--)
        {
...
{
    if(u8s->n+len+1<u8s->sz) return u8s->s;
...
char * m8s_concat(m8String * u8s, const char*s,size_t len)
{
    if(u8s->n+len+1>=u8s->sz)
        if( !m8s_realloc(u8s,len)) return NULL;
    strncpy((u8s->s+u8s->n),s,len);
    u8s->n+=len;
    u8s->s[u8s->n]='\0';
    return u8s->s;
}
char * m8s_concatc(m8String *u8s,char c)
{
    if(c==0)return u8s->s;
    if(u8s->n+1+1>=u8s->sz)
        if( !m8s_realloc(u8s,1)) return NULL;
    u8s->s[u8s->n++]=c;
    u8s->s[u8s->n]='\0';
    return u8s->s;
}
char * m8s_concati(m8String * u8s, mll i )
{
    #define MU8S_CONCATI_BUF_SZ 30
    char buf[MU8S_CONCATI_BUF_SZ],*cur;
    cur=buf+MU8S_CONCATI_BUF_SZ;
    *(--cur) = '\0';
    if(i<0)
    {
        if(!(m8s_concatcm(u8s,'-'))) return NULL;
        i=-i;
    }
    do
    {
...
int read_utf8_seq(char *stream, char ans[5],size_t len_stream )
{

    unsigned char  *cur=(unsigned char*) ans,*buf= (unsigned char*)stream;

#define MP_READ_UTF8_INTERNAL_MACRO(a) do{\
            if((*buf&0xC0)!=0x80) goto NOT_VALID_SEQ;\
            *cur++=*buf++;\
    ...
    *cur++=*buf++;
    ...
    if((unsigned char)ans[0]<0x80 )
    {
        ...
        goto NOT_VALID_SEQ;
    else if((unsigned char)ans[0]<0xF0 && ((unsigned char)ans[0]&0xF0)==0xE0 && len_stream>3)
...
{
    if(u16s->n+len+1<u16s->sz) return u16s->s;
...
mu16 * mU16s_concat(mU16String *u16s,const mu16*s,size_t len)
{
    if(u16s->n+len+1>=u16s->sz)
        if( !mU16s_realloc(u16s,len)) return NULL;

    wcsncpy(u16s->s+u16s->n,s,len);
    u16s->n+=len;
    u16s->s[u16s->n]='\0';
    return u16s->s;
}
...
{
    if(c==0) return u16s->s;
    if(u16s->n+1+1>=u16s->sz)
        if( !mU16s_realloc(u16s,1)) return NULL;
    u16s->s[u16s->n++]=c;
    u16s->s[u16s->n]='\0';
    return u16s->s;
}
...
char * mU16s_to_mU8s(mU16String *s16,m8String*s8)
{
    ...
    while(*cur)
    {
        ...
        if(! m8s_concat(s8,temp_buf,len) ) return NULL;
    }
    ...
    while (*cur)
    {
        ...
        switch (len)
        {
            case 1:
                ...
                break;
            case 2:
                c = (((unsigned int)lbuf[0] & 0x1F) << 6) 
                    + (lbuf[1] & 0x3F);
                break;
            case 3:
                c = (((unsigned int)lbuf[0] & 0xF) << 12)
                    +(((unsigned int)lbuf[1] & 0x3F)<<6)
                    + (((unsigned int)lbuf[2] & 0x3F));
                break;
            case 4:
                c = (((unsigned int)lbuf[0] & 0x7) << 18)
                    + (((unsigned int)lbuf[1] & 0x3F) << 12)
                    + (((unsigned int)lbuf[2] & 0x3F)<<6 )
                    + (((unsigned int)lbuf[3] & 0x3F));
                break;
            default :
                assert(0);
        }
...
static  const char* scan_text(const char *cur, mUString *us, const char *end,MP_ERRS *err_n )
{
    ...
    *err_n=MP_OK;
    mUs_reset(us);
    if(end-cur<1){*err_n=MP_NOT_VALID_GRAMMAR; return NULL;}
    do
    {
        cur=scan_16(cur,&uc);
        if (!mUs_concat_U16c(us, uc)) { *err_n = MP_RUN_OUT_OF_MEMORY; return NULL; }
    }while(uc && end-cur>1);

    if(uc) {*err_n=MP_NOT_VALID_GRAMMAR; return NULL;}
...
MP_ERRS mp_range_to_chset( mUString *us, mu16 start_range, mu16 end_range)
{
    ...
    {
        if(! mUs_concat_U16c(us,start_range)) return MP_RUN_OUT_OF_MEMORY;
        start_range++;
    }
     return MP_OK;
}
...
{
    mu16 r_n;
    if( *cur++!='M'){*err_n=MP_NOT_VALID_GRAMMAR; return NULL;}  //Rec Id
    cur = scan_16(cur,&r_n);
    ans->n=0;
    if(r_n>=ans->sz)
    {
    ...
    while(r_n-- && cur<=end )
    {
        ...
        switch(*cur++)
        {
            ...
            case 'S':
                ...
                if(*err_n!=MP_OK) return cur;
                if(!(l->v.s=ustrdup(ans->us->s))) {*err_n=MP_RUN_OUT_OF_MEMORY;return cur;}
                break;
            default:
...
static void destroy_mGram(mGram *g)
{
    ...
    if(g){
      ...
      {
          while(g->ndfa--)
            if(g->dfa[g->ndfa].edge) free(g->dfa[g->ndfa].edge);
          free(g->dfa);
      }
      if(g->lalr)
      {
          while(g->nlalr--)
          {
            if(g->lalr[g->nlalr].action) free(g->lalr[g->nlalr].action);
            if(g->lalr[g->nlalr].h) destroy_mHashtable(g->lalr[g->nlalr].h,NULL,NULL);
          }
      ...
      if(g->rule)
      {   while(g->nrule--)
                if(g->rule[g->nrule].symbol) free(g->rule[g->nrule].symbol);
          free(g->rule);
      }
      if(g->sym)
      {
          while(g->nsym--)
            if(g->sym[g->nsym].Name) free( (char *)g->sym[g->nsym].Name);
          free(g->sym);
      }
      if(g->chset)
      {
          while(g->ncharset--)
          {
            if(g->chset[g->ncharset].h)destroy_mHashtable(g->chset[g->ncharset].h,NULL,NULL);
            if(g->chset[g->ncharset].chset) free(g->chset[g->ncharset].chset);
          }
    ...
    while(g->n_props--)
    {
        if(g->prop_name[g->n_props])free(g->prop_name[g->n_props]);;
        if(g->prop_value[g->n_props]) free(g->prop_value[g->n_props]);
    }
    free(g->prop_name);
    free(g->prop_value);
#endif
...
static mGram * new_mGram(const char* src,size_t len,MP_ERRS *err_n )
{
    ...
    int i;
    mpField *fld;
    if(! (res= calloc(1,sizeof(mGram))))  {*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
    if(!(us=new_mUString()))           {*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
    if(!(cur=scan_text(cur,us,end,err_n))) goto ERR;

#if MP_CODING==MP_UTF8
    if(strcmp( us->s,"GOLD Parser Tables/v5.0")){*err_n=MP_NOT_VALID_GRAMMAR; goto ERR;}
#elif MP_CODING==MP_UNICODE
    if(wcscmp(us->s,L"GOLD Parser Tables/v5.0")){*err_n=MP_NOT_VALID_GRAMMAR; goto ERR;}
#endif // MP_CODING
...
#if MP_LOAD_PROPERTY
    if(! (res->prop_name=calloc(8,sizeof(muchar*)))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;};
    if(! (res->prop_value=calloc(8,sizeof(muchar*)))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;};
    res->n_props=0;
#endif
    while(cur<end)
    {
        cur=mp_read_record(&rec,cur,end,err_n);
        if( *err_n!=MP_OK) goto ERR;
        if(rec.flds[0].type!=MP_BYTE ){*err_n=MP_NOT_VALID_GRAMMAR; goto ERR;}
        switch(rec.flds[0].v.b)
        {
            ...
            fld=rec.flds+1;
            res->nsym=(fld++)->v.i;
            if(! (res->sym=calloc(res->nsym,sizeof(mSymbol)))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;} ;

            res->ncharset=(fld++)->v.i;
            if(! (res->chset=calloc(res->ncharset,sizeof(mCharSet)))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;} ;

            res->nrule=(fld++)->v.i;
            if(! (res->rule=calloc(res->nrule,sizeof(mRule)))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;} ;

            res->ndfa=(fld++)->v.i;
            if(! (res->dfa=calloc(res->ndfa,sizeof(mDfa)))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;} ;

            res->nlalr=(fld++)->v.i;
            if(! (res->lalr=calloc(res->nlalr,sizeof(mLalr)))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;} ;
            res->ngrp=fld->v.i;
            if(! (res->grp=calloc(res->ngrp,sizeof(mGroup)))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;} ;
            break;
            case 'I':
            ...
            case 'c':
                ...
                while(i--)
                {
                    if((*err_n=mp_range_to_chset(us,fld->v.i,fld[1].v.i ))!=MP_OK) goto ERR;
                    fld+=2;

                }

                if(!(res->chset[rec.flds[1].v.i].chset=ustrdup(us->s) )){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;} ;
                if(us->n>MP_SWITCH_TO_HASHTABLE_THRESHOLD)
                {
                    ...
                    if(!h){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;} ;
                    while(cur-save_start<(ptrdiff_t)us->n)
                    {
                        int n_seq;
                        n_seq=read_utf8_seq( cur,h->i_key.sbuf,us->n-(cur-save_start));
                        if(n_seq<0){*err_n=MP_NOT_VALID_GRAMMAR; goto ERR;} ;
                        cur+=n_seq;
                        if(mhash_insert(h)==MHSH_RUN_OUT_OF_MEM)
                        {
                            *err_n = MP_RUN_OUT_OF_MEMORY; goto ERR;
                        }
                        else if(h->err_n==MHSH_ERR_EXISTS)
                        {
                            printf("Char set %d contains more than once char %s"
                                   ,(int)rec.flds[1].v.i
                                   ,h->i_key.sbuf);
                        }
                    }
                    ...
                    while(*cur)
                    {
                        ...
                        {
                            printf("Char set %d contains more than once char %d",(int)rec.flds[1].v.i
                                ,(int) h->i_key.ull);
                        }
                    }
#endif
            ...
            case 'S':
                ...
                //printf(MP_STRINGPAT " %d\n",res->sym[rec.flds[1].v.i].Name,(int)res->sym[rec.flds[1].v.i].Type );
                break;
            case 'g':
                {
                    ...
                    if(gr->nnest>0)
                    {
                        ...
                        if(!tmp){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
            ...
            case 'L':
                ...
                if(i>0)
                {
                    ...
                    if(i>MP_SWITCH_TO_HASHTABLE_THRESHOLD)
                    {
                        ...
                        if(!res->lalr[rec.flds[1].v.i].h){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
                    }
                    else
                            ...
                            switch(res->lalr[rec.flds[1].v.i].h->err_n)
                            {
                                ...
                                case MHSH_ERR_EXISTS:
                                    printf("LALR %d Contains key %d more than once\n",
                                           (int)rec.flds[1].v.i,(int)tmp->SymbolIndex);
                                    break;
                                default:
    ...
    return res;
ERR:
    if(rec.flds) free(rec.flds);
    if(us) destroy_mUString(us);
    if(res)
        destroy_mGram(res);
    return NULL;
}
...
static void clean_mtree_chunk_mem(mTree * t,size_t sz)
{
    while(sz--)
    {
        if(t->token.lexeme) free((char*)t->token.lexeme);
        if(t->chs) free(t->chs);
        t++;
    }
...
void destroy_mParser(mParser * mp)
{
    ...
    if(!mp) return;
    if(mp->grm ) destroy_mGram(mp->grm);
    ...
    if(mp->lex) destroy_mUString(mp->lex);
    free(mp->tokens);
...
mParser * new_mParserA(const char * src, size_t len, MP_ERRS * err_n)
{
    ...
    mParser *res;
    if(! (res =calloc(1,sizeof(mParser)))) {*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
    *err_n=MP_OK;


    if(! (res->grm=new_mGram(src,len,err_n))) goto ERR;


/**treeData**/
    if(!(res->tree_data.mem_chunks=new_mStack(5) ) ){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
    res->tree_data.sz=MP_TREE_MEM_CHUNK_SZ;
    res->tree_data.n=0;
    if(! (res->tree_data.mem_chunks->io_val.v
          =res->tree_data.cur_buf
          =calloc(res->tree_data.sz,sizeof(mTree)))) {*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
    mStack_push(res->tree_data.mem_chunks);
    if(!(res->tree_stack=new_mStack(5))){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;};

    res->sztoks=MP_TOKEN_BUF_SZ;
    res->ntokens=0;
    if( !(res->tokens=calloc(res->sztoks,sizeof(mToken)) )){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
    if(!(res->lex=new_mUString())){*err_n=MP_RUN_OUT_OF_MEMORY; goto ERR;}
return res;
ERR:
    if(res) destroy_mParser(res);
    return NULL;
}
mParser * new_mParserF(const char* file_name, MP_ERRS * err_n  )
{
    ...
    size_t src_len;
    mParser *res=NULL;
    if( (*err_n=mp_loadfile(file_name,&gt,&src_len))!=MP_OK) return NULL;
    if(gt)
    {
...
static short mp_scan(mParser * mp)
{
    ...
    if (mp_next_char(mp)!=MP_OK ) return -1; // it can raise  not valid utf8 seq
    if (mp->in.MEOF ) return 0;
    ...
    while (1)
    {
        ...
        {
#if MP_CODING==MP_UTF8
            sprintf(_err_buf, MP_ERR_MSG_UNEXPECTED_CHAR, mp->in.cur, mp->in.row_n, (int)(1 + (mp->in.cur - mp->in.start_row_pos)));

#elif MP_CODING==MP_UNICODE
            swprintf(_err_buf, (sizeof(_err_buf)/ sizeof(*_err_buf)) - 1,
                MP_ERR_MSG_UNEXPECTED_CHAR,
                mp->in.cur,
                mp->in.row_n,
...
#elif MP_CODING==MP_UNICODE
                    swprintf(_err_buf, (sizeof(_err_buf)/sizeof(*_err_buf)) - 2, MP_ERR_MSG_UNKNOWN_ERR, (int)grm->sym[last_accepted].Type);
...
static int mparser_parse( mParser * mp)
{
    ...
    if (mp->reduction)
    {
        ...
        while(1)
        {
            if (mp->ntokens< 1)
            {
                if ((mp->symbol= mp_scan(mp)) < 0) return -1;
        ...
        else
        {
...
#if MP_CODING==MP_UTF8
                sprintf(_err_buf,
                       MP_ERR_MSG_UNEXPECTED_TK,
                        tk->lexeme,
                        mp->in.row_n,
                        (int)(1 + (mp->in.cur - mp->in.start_row_pos) - ustrlen(tk->lexeme)));
#elif MP_CODING==MP_UNICODE
                swprintf(_err_buf,
                    (sizeof(_err_buf) / sizeof(*_err_buf)) - 2,
                    MP_ERR_MSG_UNEXPECTED_TK,
                    tk->lexeme,
                    mp->in.row_n,
                    (int)(1 + (mp->in.cur - mp->in.start_row_pos) - ustrlen(tk->lexeme)));
            ...
            switch(a->Action)
            {
                case ActionShift:
                    ...
                    sy=mp->grm->sym+mp->symbol;
                    if(!mp_push_new_leaf(mp,sy,tk->lexeme,NULL,0)) return -1;
                    assert(mp->ntokens);
                    mp->tokens[--mp->ntokens].lexeme=NULL;
